...
cl_device_id oclGetMaxFlopsDev(cl_context cxGPUContext)
{
    ...
    while( current_device < device_count )
    {
        ...
        // CL_DEVICE_MAX_CLOCK_FREQUENCY
        cl_uint device_clock_frequency;
        clGetDeviceInfo(cdDevices[current_device], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(device_clock_frequency), &device_clock_frequency, NULL);

        int flops = device_compute_units * device_clock_frequency;
        if( flops > max_flops )
        {
            max_flops        = flops;
